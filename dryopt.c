...
opt_could_be_boolean(struct dryopt const *const opt)
{
...
// derived from negate_boolean_opt() below
{
	return opt_could_be_boolean(opt)
...
static bool
negate_boolean_opt(struct dryopt const *const opt)
{
	assert(opt_could_be_boolean(opt));
...
found:		if (!(opt_could_be_boolean(opts + opti) && negate_boolean_opt(opts + opti)))
